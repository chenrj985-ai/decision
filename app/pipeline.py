...
def run() -> int:
    ...
    dimensions = market_dimensions(index_df, stock_df, etf_scores, risk)
    scored, _, human_view = apply_trading_coach(
        scored, etf_scores, risk, mode, config
    )
    ...
    if buys.empty:
        ...
        for _, row in buys.iterrows():
            lines.append(
                f"{row['name']}｜{row['candidate_grade']}级｜ETF {row['etf_grade']}｜"
                f"教练分{row['coach_score']}｜爆雷{row['explosion_index']}｜"
                f"{row['coach_action']}"
            )
